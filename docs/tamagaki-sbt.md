...
Native BitcoinとLightningは例外です。初期Bitcoinでは支援者が送金前に予定額、Base address、玉垣hash、nonceを署名し、送金後に複数の独立検証者がbeneficiary VASPの認証済み入金記録と`txid:vout`を照合します。検証者AttestationがIntent hash、outpoint、実受領額、confirmationを拘束し、Base Registryへ登録した後にBase版SBTを発行します。Lightningは将来経路で、元のpayment hashを公開しません。Bitcoinの支払いとSBT発行は非原子的です。
