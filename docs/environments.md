...
| ネットワーク | Hardhatローカル、Sepolia、Base Sepolia、ローカルBitcoin Core／LND regtest。Signet／testnetは次段階の統合試験 | ETHはBase、JPYCはPolygon、初期BTCは国内登録VASPを受取側とするVASP間Native Bitcoin、Bitcoin由来SBTはBase、Base ETHにはEthereum L1回収経路。自己管理walletとLightningは後日有効化 |
