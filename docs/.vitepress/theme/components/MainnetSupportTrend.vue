...
<template>
  <section class="support-trend mainnet-trend" aria-labelledby="mainnet-support-title">
    ...
    <div class="multichain-summary mainnet-summary">
      ...
      <div class="multichain-summary__row"><strong>Bitcoin Mainnet<small>Bitcoin network</small></strong><span><small>{{locale === "ja" ? "資産" : "Asset"}}</small>BTC</span><span><small>{{locale === "ja" ? "支援総額" : "Total"}}</small>—</span><span><small>{{locale === "ja" ? "支援件数" : "Contributions"}}</small>—</span><span><small>{{locale === "ja" ? "状態" : "Status"}}</small>{{locale === "ja" ? "受付開始前" : "Not launched"}}</span></div>
    </div>
    ...
    <div v-else class="support-trend__empty"><strong>{{locale === "ja" ? "メインネットでの支援募集はまだ開始していません" : "Mainnet fundraising has not started"}}</strong><span>{{locale === "ja" ? "テストネットの支援は含めません。Base ETH、Polygon JPYC、Bitcoinを別資産として表示します。" : "Testnet contributions are excluded. Base ETH, Polygon JPYC, and Bitcoin are reported as separate assets."}}</span></div>
    <section class="mainnet-tamagaki">
      <div class="tamagaki-explorer__heading"><div><p class="whitepaper-hero__eyebrow">MAINNET TAMAGAKI / ALL CHAINS</p><h2>{{locale === "ja" ? "熊本城を囲む本番玉垣SBT" : "Mainnet Tamagaki around Kumamoto Castle"}}</h2><p>{{locale === "ja" ? "Base、Polygon、将来のBitcoin由来SBTを一つの垣根として表示します。" : "Base, Polygon, and future Bitcoin-derived SBTs form one shared fence."}}</p></div><strong>{{sbts.length.toLocaleString()}}<small>{{locale === "ja" ? "本" : " Tamagaki"}}</small></strong></div>
      ...
      <div v-else class="support-trend__empty"><strong>{{locale === "ja" ? "本番玉垣SBTはまだ発行されていません" : "No mainnet Tamagaki have been issued"}}</strong><span>{{locale === "ja" ? "本番SBTアドレスの設定後、Base、Polygon、Bitcoin由来SBTを統合して表示します。" : "After production SBT addresses are configured, Base, Polygon, and Bitcoin-derived SBTs appear together."}}</span></div>
    </section>
