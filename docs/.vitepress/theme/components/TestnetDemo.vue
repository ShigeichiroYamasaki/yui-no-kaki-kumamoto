...
const rpc = (import.meta.env.VITE_RECOVERY_RPC_URL as string | undefined) || "https://sepolia.base.org";
...
async function wallet() { const p=provider(); try { await p.request({method:"wallet_switchEthereumChain",params:[{chainId:"0x14a34"}]}); } catch { await p.request({method:"wallet_addEthereumChain",params:[{chainId:"0x14a34",chainName:"Base Sepolia",nativeCurrency:{name:"ETH",symbol:"ETH",decimals:18},rpcUrls:[rpc],blockExplorerUrls:["https://sepolia-explorer.base.org"]}]}); } return createWalletClient({account:account.value,chain:baseSepolia,transport:custom(p)}); }
async function connect(){ try { const addresses=await provider().request({method:"eth_requestAccounts"}) as Address[]; account.value=addresses[0]; await wallet(); await refreshBalance(); message.value="Base Sepoliaに接続しました"; } catch(e){message.value=e instanceof Error?e.message:String(e);} }
...
<template><section class="testnet-demo">
  <div class="demo-warning"><b>BASE SEPOLIA TESTNET</b> 実資産ではありません。MockJPYCは公式JPYCではなく、このデモ専用トークンです。</div>
  ...
  <template v-else>
    ...
    <div class="demo-faucets"><a href="https://docs.base.org/base-chain/network-information/network-faucets" target="_blank" rel="noreferrer">Base Sepolia ETH Faucet ↗</a><button :disabled="!account||!!busy" @click="faucet">MockJPYCを100,000受け取る</button><span>残高 {{balance}} mJPYC</span></div>
    ...
    <p class="demo-result">{{message}} <a v-if="lastTx" :href="`https://sepolia-explorer.base.org/tx/${lastTx}`" target="_blank" rel="noreferrer">Explorerで確認 ↗</a><strong v-if="tokenId"> 玉垣SBT #{{tokenId.toString()}}</strong></p>
  </template>
</section></template>
