...
`BitcoinSupportRegistry` verifies a supporter-signed EIP-712 `SupportIntent` and threshold verifier signatures over an `Attestation`, then mints the Tamagaki SBT in the same Base transaction. A Bitcoin txid/vout or domain-separated Lightning payment commitment is a single-use evidence key. Signatures are bound to the Base chain ID, Registry address, and verifier epoch. Bitcoin amounts use satoshis and eight decimals; Lightning amounts use millisatoshis and eleven decimals. Payment hashes, preimages, invoices, and macaroons never enter the Registry.
...
This is only the Base-side evidence and issuance prototype. Native BTC cannot be sent to existing EVM Vaults. Bitcoin Core watch-only intake, unique-address derivation, confirmation/reorganization monitoring, PSBT operations, the LND invoice service, Paymaster, indexer, and UI remain unimplemented. Native Bitcoin requires Signet/testnet-to-Base-Sepolia end-to-end testing and audit. Lightning additionally requires online-signer exception validation and a separate launch approval.
