...
The repository contains guarded Base Mainnet and Polygon Mainnet deployment modules and network configuration, but no production addresses have been deployed. The Base escape hatch, Polygon recovery runbook, and production indexer remain unimplemented. The Base-side Bitcoin/Lightning threshold-attestation Registry is implemented as a prototype, while Bitcoin Core/LND intake services remain unimplemented. Current EVM SBTs mint on their contribution chain; Bitcoin/Lightning contributions mint on Base only after threshold attestation. The indexer uses `chainId:sbtContract:tokenId` to combine records.
...
The home page uses read-only Viem Public Clients and refreshes `SupportReceived` events every 30 seconds. Base ETH, Polygon JPYC, and Bitcoin BTC appear as three rows of one table. The Bitcoin row combines Native Bitcoin and enabled Lightning in BTC with a route breakdown. Until Bitcoin intake and its indexer are configured and opened, the row shows a not-yet-open state rather than an estimated amount. Configure `BASE_MAINNET_PUBLIC_RPC_URL`, `BASE_MAINNET_VAULT_ADDRESS`, `BASE_MAINNET_TAMAGAKI_SBT_ADDRESS`, and `BASE_MAINNET_DEPLOYMENT_BLOCK` for Base, plus `POLYGON_MAINNET_PUBLIC_RPC_URL`, `POLYGON_MAINNET_VAULT_ADDRESS`, `POLYGON_MAINNET_TAMAGAKI_SBT_ADDRESS`, `POLYGON_MAINNET_JPYC_ADDRESS`, `POLYGON_MAINNET_DEPLOYMENT_BLOCK`, and `POLYGON_MAINNET_JPYC_DECIMALS` for Polygon. The home-page mainnet Tamagaki block currently combines SBTs from both EVM production chains into one fence and will later add Bitcoin-derived Base SBTs under ADR-0011. Testnet totals and SBTs remain isolated on the demo status page.

### Bitcoin and Lightning Base Registry prototype

`BitcoinSupportRegistry` version 2 uses a two-stage signature model. The pre-payment EIP-712 `SupportIntent` contains only route, intended amount, Base recipient, artwork hash, expiry, and nonce. After payment, verifier signatures bind the intent hash to a Bitcoin `txid:vout` or Lightning public commitment, actual amount, and confirmation reference. The contract requires route and amount agreement, threshold signatures, verifier epoch, and unique nonce and evidence. Expiry applies to the attested payment-observation time rather than the later mint time, so VASP review delay alone does not invalidate an in-time payment. The EIP-712 domain version is `2`.

Final aggregation treats valid `SupportAttested` events as the sole monetary source of truth and never adds `BitcoinTamagakiIssued` or the ERC-721 mint again. Records followed by `SupportInvalidated` are excluded from final amount and count. The current contract advances `None → Accepted → SBTIssued` in one transaction. Persistence of `Detected`, `Confirmed`, and Lightning settlement plus verification of source evidence belong to the still-unimplemented indexer and verifier services; the contract does not directly verify Bitcoin or Lightning.

`lib/bitcoin-support.ts` provides shared txid validation, Lightning public-commitment generation, artwork hashing, and verifier-signature ordering. Deploy the Base Sepolia prototype with `ignition/modules/BitcoinSupportBase.ts` after copying `ignition/parameters/bitcoin-base-sepolia.example.json` to the gitignored non-example filename and replacing all placeholder roles.

This is only the Base-side evidence and issuance prototype. The service that consumes authenticated VASP deposit records and reconciles public outpoints, plus the Paymaster, indexer, and UI, remain unimplemented. Future direct-custody Bitcoin Core, unique-address derivation, PSBT, and LND are separate later components. Because the ABI and EIP-712 domain changed, deploy a new Registry rather than reconciling this version with an old Ignition deployment.
...
For Bitcoin/Lightning demo aggregation, configure the separately deployed `BitcoinSupportBaseModule` as `BASE_SEPOLIA_BITCOIN_REGISTRY_ADDRESS`, `BASE_SEPOLIA_BITCOIN_SBT_ADDRESS`, and `BASE_SEPOLIA_BITCOIN_DEPLOYMENT_BLOCK`. The view treats valid `SupportAttested` events as canonical, excludes `SupportInvalidated`, and joins token IDs from `BitcoinTamagakiIssued` to the dedicated SBT contract. The Bitcoin demo row remains absent until all three values are present.

Browser aggregation reads `eth_getLogs` sequentially in ranges of at most 10,000 blocks and retries only 429/rate-limit responses with exponential backoff. If the configured endpoint is unavailable, it falls back to a read-only public RPC for the same chain. A throttled network does not erase the last successful values or data from another network, and the UI does not expose the RPC URL or request body in its error text. Any RPC URL bundled into Pages is public, so provider keys must have domain restrictions, usage caps, and a read-only project scope.
